			...
			for (Coursed coursed : student.getCurriculum().getCoursedCourses()) {
				...
				if (addCourse) {
					...
					if (coursed.getSemester() == AcademicSemester.BOTH) {
						if (!(semestersOfCourses.contains(parseYearSemesterToString(coursed.getYear(), AcademicSemester.FIRST))) ||
								!(semestersOfCourses.contains(parseYearSemesterToString(coursed.getYear(), AcademicSemester.SECOND)))) {
							semestersOfCourses.add(parseYearSemesterToString(coursed.getYear(), AcademicSemester.FIRST));
							semestersOfCourses.add(parseYearSemesterToString(coursed.getYear(), AcademicSemester.SECOND));
						}
					} else {
						if (!(semestersOfCourses.contains(parseYearSemesterToString(coursed.getYear(), coursed.getSemester())))) {
		...
		for (Course course : Manager.INSTANCE.courses) {
			...
			if (addCourse) {
				...
				if (course.getSemester() == AcademicSemester.BOTH) {
					if (!(semestersOfCourses.contains(parseYearSemesterToString(Manager.getYear(), AcademicSemester.FIRST))) ||
							!(semestersOfCourses.contains(parseYearSemesterToString(Manager.getYear(), AcademicSemester.SECOND)))) {
					...
					if (!(semestersOfCourses.contains(parseYearSemesterToString(Manager.getYear(), course.getSemester())))) {
	...
	void showCourses(String newValue) {
		...
		if (year == Manager.getYear() && semester == Manager.INSTANCE.currentSemester.getSemester()) {
			...
			for (Coursed coursed : myCoursedCourses) {
				if (coursed.getYear() == year && (coursed.getSemester() == semester || coursed.getSemester() == AcademicSemester.BOTH)) {
