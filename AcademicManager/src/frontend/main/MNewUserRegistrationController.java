...
public class MNewUserRegistrationController extends MViewController {
	...
	public void setUp() {
		...
		String[] accesos = new String[] { Messages.getUILabel(UILabel.STUDENT), Messages.getUILabel(UILabel.PROFESSOR),
				Messages.getUILabel(UILabel.ADMINISTRATOR) };

		chBxAccess.setItems(FXCollections.observableArrayList(accesos));
		chBxSex.setItems(FXCollections.observableArrayList(Messages.getUILabel(UILabel.MALE),
				Messages.getUILabel(UILabel.FEMALE)));
		chBxSex.getSelectionModel().selectFirst();

		chBxAccess.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
			@Override
	...
	public void btnContinue_Pressed() {
		...
		Gender gender = chBxSex.getSelectionModel().getSelectedItem().equals(Messages.getUILabel(UILabel.MALE))
		...
		if (isAdmin) {
			...
			Professor newProfessor = new Professor(rut, name, lastFather, lastMother, address, gender, number,
					birthday);
			...
			Student newStudent = new Student(id, year, studyPrograms, rut, name, lastFather, lastMother, address,
