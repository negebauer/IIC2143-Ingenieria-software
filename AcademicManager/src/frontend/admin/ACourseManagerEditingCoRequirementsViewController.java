...
public class ACourseManagerEditingCoRequirementsViewController extends MCourseSearcherSelectorViewController {
	...
	public void btnAddCoRequirement_Pressed() {
		if (!chBxSelectedCourse.getSelectionModel().isEmpty() & chBxSelectedCourse.getItems().contains(chBxSelectedCourse.getSelectionModel().getSelectedItem())) {		
			...
			//ViewUtilities.showAlert(Messages.getUILabel(UILabel.ERROR_SELECTION) + "(" + Messages.getUILabel(UILabel.ADD_CO_REQUIREMENT) + ")");
		}
	}
	...
	public void btnRemoveCoRequirement_Pressed() {
		if (!listCoRequirements.getSelectionModel().isEmpty()) {	
			...
			// TODO Uncomment when function is created
			//ViewUtilities.showAlert(Messages.getUILabel(UILabel.ERROR_SELECTION) + "(" + Messages.getUILabel(UILabel.REMOVE_CO_REQUIREMENT) + ")");
		}
	}	
