	...
	public void btnSaveCourse_Pressed() {
		...
		if (isLecture) {
			...
			for (Professor prof : Manager.INSTANCE.professors) {
				if (AssistantsOrProfessorsStrings.contains(prof.getName() + " " + prof.getLastnameFather() + " " + prof.getLastnameMother())) {
					professors.add(prof);
				}
			}
			...
			for (Assistant assist : Manager.INSTANCE.assistants) {
				if (AssistantsOrProfessorsStrings.contains(assist.getName() + " " + assist.getLastnameFather() + " " + assist.getLastnameMother())) {
