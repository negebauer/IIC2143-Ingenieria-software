...
public class CourseCoRequirementsReaderWriter {
	...
	/* File format
	initials course&initials co requirement
	*/
	
	/**
	 * Writes all the course co requirements to the .txt file.
	 * @param allCourse All the courses to write their co requirements.
	 */
	...
	/**
	 * Reads all the course co requirements from the .txt file and adds them to each corresponding course.
			...
			while (courseCoRequirementString != null ) {
				...
				for (Course course : allCourses) {
					for (Course courseCoRequired : allCourses) {
						if (course.getInitials().equals(courseInitials) && courseCoRequired.getInitials().equals(courseCoRequiredInitials)) {
